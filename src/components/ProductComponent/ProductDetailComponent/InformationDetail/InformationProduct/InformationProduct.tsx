        ...
        {valueproduct
          ...
          ?.map((item: any, index: any) => {
            return (
              <div
                key={index}
                className="xl:w-8/12 w-full mr-3 sm:flex sm:flex-col"
              >
                    ...
                    <li className="mb-2">
                      <p className="text-sm">
                        ...
                        <span className="text-[#ee1923] text-xl font-semibold">
                          {VND(item?.promotionalMarketPrice1)
                            ?.toString()
                            ?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" "}
                        </span>
                      </p>
                    ...
                    <li className="text-sm mb-2">
                      <p className="text-sm">
                        ...
                        <span className="text-lg text-[#ee1923] font-semibold">
                          {VND(item?.promotionalMarketPrice2)
                            ?.toString()
                            ?.replace(/\B(?=(\d{3})+(?!\d))/g, ",")}{" "}
                        </span>
                      </p>
                ...
                <div className="my-2">
                  ...
                  <div>
                    <ul className="w-full grid grid-cols-4 gap-4">
                      {valueproduct?.option && valueproduct?.option?.map((itemModel: any, key: any) => {
                        return (
                          <li
                            key={key}
                ...
                <div className="mt-8">
                  <div className="flex justify-between sm:mb-3">
                    <div
                      onClick={() => onClickByProduct(item)}
                      className="sm:mr-2 mr-3 border border-solid hover:bg-black hover:text-white cursor-pointer border-black w-1/2 !h-auto flex justify-center items-center flex-col p-2 rounded-md"
                    >
                      <b className="text-base">Mua ngay</b>
                      <span>Giao hàng tận nới</span>
                    </div>
                    <div
                      onClick={() => onClickAddCartProduct(item)}
                      className="bg-blue-500 ml-3 hover:bg-blue-700 cursor-pointer text-white w-1/2 !h-auto flex justify-center items-center flex-col p-2 rounded-md"
                    >
                  ...
                  <div className="w-full mt-1">
                    <div className="bg-red-500 hover:bg-red-700 cursor-pointer text-white w-full !h-auto flex justify-center items-center flex-col p-2 rounded-md ">
                      ...
                      <span className="text-center">
                        (Tư vấn viên gọi lại cho quý khách trong vòng 5 phút)
                      </span>
                    </div>
                  </div>
                </div>
